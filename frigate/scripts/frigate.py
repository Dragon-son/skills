...
def api_url(path: str) -> str:
    return f"{FRIGATE_URL.rstrip('/')}{path}"


def login():
    """Authenticate with Frigate and return a session.

    Some Frigate deployments expose read endpoints directly even when
    /api/login is unavailable or misconfigured. In that case, fall back
    to an unauthenticated session so read-only operations still work.
    """
    ...
    try:
        ...
        if response.status_code == 200:
            return session
        print(
            f"Warning: login failed ({response.status_code}), continuing with direct API access.",
            file=sys.stderr,
        )
        return session
    except requests.RequestException as exc:
        print(
            f"Warning: login request failed ({exc}), continuing with direct API access.",
            file=sys.stderr,
        )
        return session
...
def list_cameras(session):
    """List all cameras."""
    config = session.get(api_url("/api/config"), verify=False).json()
...
def get_snapshot(session, camera_name, output_path=None):
    ...
    response = session.get(
        api_url(f"/api/{camera_name}/latest.jpg"),
        verify=False
    )
    if response.status_code != 200:
